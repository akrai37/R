...
merged <- merge(stats, mydf ,by.x="Country.Code", by.y="Code") #'by.x' and 'by.y' represent the common columns to join. Just like in SQL.          
head(merged)
...
#2.Transparency
qplot(data=merged, x=Internet.users, y=Birth.rate ,color=Region, size=I(4), shape=I(19),
      alpha=0.6)
